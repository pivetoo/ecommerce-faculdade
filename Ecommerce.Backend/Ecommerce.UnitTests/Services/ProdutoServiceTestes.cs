...
namespace Ecommerce.UnitTests.Services
{
    public class ProdutoServiceTests
    {
        ...
        [Test]
        public async Task CriarNovoProduto()
        {
            var produto = new Produto { Id = 1, Nome = "Produto Teste", Descricao = "Descricao Teste", Preco = 15, Estoque = 8 };
            _produtoRepositoryMock.Setup(repo => repo.Incluir(It.IsAny<Produto>())).Returns(Task.CompletedTask);

            var result = await _produtoService.CriarNovoProduto(produto);

            _produtoRepositoryMock.Verify(repo => repo.Incluir(produto), Times.Once);
